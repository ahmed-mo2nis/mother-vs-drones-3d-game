...
var splash_object = preload("res://Scenes/Rain.tscn")
var splashes = []
...
func make_splash():
	...
	if result.size() > 0:
		splashes[cur_splash_ind].global_transform.origin = result.position + Vector3(0, 0.5, 0)
